...
export class CartService {
  ...
  getCartItemByProductId(productId: number): CartItem | undefined {
    return this.cartItems.find((item) => item.product.id === productId && item.userId === this.userId);
  }
  /**
   * Get the cart items
  ...
  removeFromCartByProductId(productId: number): Observable<any> {
    ...
    return this.deleteCartItemFromServer(cartItem.id).pipe(
      tap(() => {
        const itemIndex = this.cartItems.findIndex((item) => item.id === cartItem.id);
        if (itemIndex !== -1) {
          this.cartItems.splice(itemIndex, 1);
